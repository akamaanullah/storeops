...
$myAssignedView = $myAssignedView ?? false;
$jobsRoute = $jobsRoute ?? '/jobs';
?>

<!-- Header controls -->
<div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
    <div>
        <h1 class="text-2xl font-serif italic text-natural-heading tracking-tight"><?= $myAssignedView ? 'My Assigned Jobs' : 'Work Orders' ?></h1>
        <p class="text-xs text-natural-darkmute mt-1"><?= $myAssignedView ? 'Work orders currently assigned to you' : 'Search and filter all work orders' ?></p>
    </div>
...
<div class="bg-white p-6 border border-natural-border rounded-3xl shadow-sm text-xs mb-6">
    <form action="<?= BASE_URL . $jobsRoute ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 text-natural-text leading-relaxed font-normal">
        <!-- Search field -->
        ...
        <div class="md:col-span-4 flex justify-between items-center mt-5 pt-3 border-t border-natural-border">
            <a href="<?= BASE_URL . $jobsRoute ?>" class="font-bold text-natural-muted hover:text-natural-primary">Clear all filters</a>
