...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-3xl border border-natural-border shadow-sm mb-8">
  <div class="min-w-0 flex-1">
    <h1 class="text-2xl font-serif italic text-natural-heading tracking-tight leading-tight">My Dashboard</h1>
    <p class="text-xs text-natural-darkmute mt-1">Overview of your assigned jobs and pending work · <span class="font-mono text-[9px] font-bold text-natural-primary uppercase bg-natural-subtle px-2 py-0.5 border border-natural-border rounded-xl ml-1"><?= htmlspecialchars(RoleLabels::label($user['role'] ?? null)) ?></span></p>
  </div>
...
<div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
  <div class="bg-white p-5 rounded-3xl border border-natural-border shadow-sm flex items-center justify-between col-span-2 lg:col-span-1">
    <div>
      <span class="text-[9px] font-bold text-natural-muted uppercase tracking-widest block leading-none font-mono">My Jobs</span>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
  ...
  <div class="bg-white border border-natural-border rounded-3xl shadow-sm overflow-hidden lg:col-span-2 flex flex-col">
    <div class="px-6 py-5 border-b border-natural-border flex justify-between items-center bg-natural-pane/30">
      <div>
        <h2 class="font-bold text-natural-heading tracking-tight text-sm">My Assigned Jobs</h2>
        <p class="text-[10px] text-natural-muted font-medium">A summary of your current job assignments</p>
      </div>
      <a href="<?= BASE_URL ?>/jobs/mine" class="text-xs text-natural-primary font-bold hover:underline mb-0.5">View all assigned &rarr;</a>
    </div>
    <div class="overflow-x-auto">
      <?php if (empty($recentJobs)): ?>
        <div class="p-10 text-center text-xs text-natural-muted">No jobs have been assigned to you yet.</div>
      <?php else: ?>
