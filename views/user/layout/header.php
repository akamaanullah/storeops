...
<body class="min-h-screen text-natural-text flex flex-col md:flex-row bg-natural-bg pb-20 md:pb-0">
    ...
    <?php if ($currentUser): ?>
        ...
        <aside class="hidden md:flex flex-col w-64 h-screen sticky top-0 bg-white border-r border-natural-border justify-between py-6 px-4 shrink-0 z-50">
            <!-- Brand Section -->
            <div class="space-y-6 flex flex-col flex-1 min-h-0">
                <a href="<?= BASE_URL ?>/" class="flex items-center space-x-3 focus:outline-none px-2">
                    <?php $brandLogoSize = 'sidebar'; include dirname(__DIR__, 2) . '/partials/app-brand-logo.php'; ?>
                    <span class="font-serif italic text-natural-heading text-lg tracking-tight"><?= htmlspecialchars(APP_NAME) ?></span>
                </a>
                
                <!-- Navigation + sidebar widgets -->
                <div class="flex flex-col flex-1 overflow-y-auto min-h-0">
                    <?php include dirname(__DIR__, 2) . '/partials/user-sidebar-content.php'; ?>
                </div>
            </div>
            ...
            <div class="space-y-4 px-2">
                ...
                <a href="<?= BASE_URL ?>/auth/logout" class="flex items-center space-x-3 px-3 py-2.5 text-xs font-semibold rounded-xl text-natural-darkmute hover:text-rose-600 hover:bg-rose-50/50 transition-all focus:outline-none animate-fade-in" title="Sign out">
